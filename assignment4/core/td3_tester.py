...
class TD3Trainer:
    ...
    def load(self, filename):
        self.critic.load_state_dict(torch.load(filename + "_critic", map_location=lambda storage, loc: storage.cuda(0)))
...
if __name__ == "__main__":
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if args.load_model:
            ...
            policy.load(f"{log_dir}/models/filename")
        ...
        if done:
            ...
            episode_num += 1
        if episode_num > 50:
            print("aver r:", np.mean(np.array(episode_reward_list)))
            break
